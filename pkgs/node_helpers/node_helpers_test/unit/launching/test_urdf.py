import pytest
from node_helpers.launching import urdf
from node_helpers.launching.urdf import NAMESPACE_FMT
...
def test_fix_urdf_paths_makes_path_replacements() -> None:
    ...
    modified_urdf = urdf.fix_urdf_paths(
...
def test_assert_attributes_exist_fails_on_duplicate_attributes_across_urdfs() -> None:
    ...
    urdf_text = GENERIC_URDF.read_text()
    urdf_text_namespaced = urdf.prepend_namespace(GENERIC_URDF.read_text(), "cool")
    ...
    with pytest.raises(ValueError):
        urdf.assert_link_names_exist([urdf_text, urdf_text], EXPECTED_LINK_NAMES)

    # This should fail because the joint names don't actually exist
    with pytest.raises(ValueError):
        urdf.assert_link_names_exist(
            [urdf_text_namespaced, urdf_text_namespaced], EXPECTED_LINK_NAMES
        )

    # Test happy path with just one urdf file
    urdf.assert_link_names_exist([urdf_text_namespaced], _expected_links_namespaced)
    urdf.assert_link_names_exist([urdf_text], EXPECTED_LINK_NAMES)
    ...
    urdf.assert_link_names_exist([urdf_text, urdf_text_namespaced], EXPECTED_LINK_NAMES)
    urdf.assert_link_names_exist(
...
def test_assert_link_names_exist() -> None:
    ...
    # Raises error when no names provided
    with pytest.raises(ValueError):
        urdf.assert_link_names_exist([urdf_text], [])
...
def test_prepend_namespace() -> None:
    ...
    modified = urdf.prepend_namespace(urdf_text, namespace=namespace)
    ...
    for changes in (EXPECTED_JOINT_NAMES, EXPECTED_LINK_NAMES):
        ...
        actual_changes = list(
            map(
                modified.count,
                [
                    urdf.NAMESPACE_FMT.format(namespace=namespace, name=j)
                    for j in changes
                ],
            )
        )
