...
def image_to_numpy(msg: Image) -> npt.NDArray[np.float64]:
...
def numpy_to_image(arr: npt.NDArray[np.float64], encoding: str) -> Image:
    ...
    if exp_channels != channels:
        raise TypeError(
            f"Array has {channels} channels, {encoding} requires {exp_channels}"
        )
